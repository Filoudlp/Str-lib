...
@dataclass
class ConcreteCoefficients:
    """
    Coefficients réglementaires pour le béton selon l'Annexe Nationale.

    :param alpha_cc: Coefficient pour fcd [-]
    :param alpha_cc_pl: Coefficient alpha_cc pour planchers [-]
    :param alpha_ct: Coefficient pour fctd [-]
    :param alpha_ct_pl: Coefficient alpha_ct pour planchers [-]
    :param gamma_c: Coefficient partiel de sécurité [-]
...
_NATIONAL_ANNEX_REGISTRY: dict[str, ConcreteCoefficients] = {
    "FR": ConcreteCoefficients(
        alpha_cc=1.0,
        alpha_ct=1.0,
        gamma_c=1.5, country="FR"
    ),
    "BE": ConcreteCoefficients(
        alpha_cc=0.85,
        alpha_ct=0.85,
        gamma_c=1.5, country="BE"
    ),
    "UK": ConcreteCoefficients(
        alpha_cc=0.85,
        alpha_ct=1.0,
        gamma_c=1.5, country="UK"
    ),
}
...
class MatConcrete(Material):
    ...
    """
    BA_TYPE = {8: "C8/10", 12: "C12/15", 16: "C16/20",
               20: "C20/25", 25: "C25/30",
               30: "C30/37", 35: "C35/45",
               40: "C40/50", 45: "C45/55",
               50: "C50/60", 55: "C55/67",
    ...
    def __init__(self, fck: float, country: str = "FR",
        ...
        super().__init__(
            name= self.BA_TYPE[self._fck] if self._fck in self.BA_TYPE else f"C{self._fck:.0f}",
            E=self._ecm,
            nu=self._DEFAULT_NU,
    ...
    def _internal_propserty(self, fck : float) -> None:
        ...
        self._epsilon_c2 = max(2.0, 2.0 +0.085 * (self._fck - 50) ** 0.53) / 1000
    ...
    @property
    def fcd(self) -> float:
        return self._coefficients.alpha_cc * self._fck / self._coefficients.gamma_c
    ...
    def fcd_report(self) -> FormulaResult:
        ...
        return FormulaResult(
            ...
            formula_values=(
                f"fcd = {c.alpha_cc} × {self._fck:.2f} / {c.gamma_c} "
                f"= {self.fcd:.2f}"
            ),
            result=self.fcd,
            unit="MPa",
    ...
    @property
    def fctd(self) -> float:
        return self._coefficients.alpha_ct * self._fctk_005 / self._coefficients.gamma_c
    ...
    def fctd_report(self) -> FormulaResult:
        ...
        return FormulaResult(
            ...
            formula_values=(
                f"fctd = {c.alpha_ct} × {self._fctk_005:.2f} / {c.gamma_c} "
                f"= {self.fctd:.2f}"
            ),
            result=self.fctd,
            unit="MPa",
