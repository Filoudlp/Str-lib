...
@dataclass
class ReinforcementCoefficients:
    """
    Coefficients partiels de sécurité pour les armatures béton armé
    selon l'Annexe Nationale — EC2 §2.4.2.4.

    :param gamma_s: Coefficient partiel armatures à l'ELU [-]
    :param gamma_s_acc: Coefficient partiel armatures accidentel [-]
    :param country: Code pays
    """
    gamma_s: float = 1.15
    gamma_s_acc: float = 1.00
    country: str = "FR"


# Registre des Annexes Nationales
_NATIONAL_ANNEX_REGISTRY: dict[str, ReinforcementCoefficients] = {
    "FR": ReinforcementCoefficients(gamma_s=1.15, gamma_s_acc=1.00, country="FR"),
    "BE": ReinforcementCoefficients(gamma_s=1.15, gamma_s_acc=1.00, country="BE"),
    "UK": ReinforcementCoefficients(gamma_s=1.15, gamma_s_acc=1.00, country="UK"),
}
...
class MatReinforcement(Material):
    ...
    def __init__(
        ...
        # Coefficients AN
        self._coefficients: ReinforcementCoefficients = _NATIONAL_ANNEX_REGISTRY[country]

        # Nom pour affichage
        self._name: str = name or f"B{int(fyk)}{ductility_class}"
    ...
    def country(self, value: str) -> None:
        ...
        value = value.upper()
        if value not in _NATIONAL_ANNEX_REGISTRY:
            raise ValueError(
                f"Annexe Nationale '{value}' inconnue. "
                f"Choix possibles : {list(_NATIONAL_ANNEX_REGISTRY.keys())}"
            )
        self._country = value
        self._coefficients = _NATIONAL_ANNEX_REGISTRY[value]
    ...
    def gamma_s(self) -> float:
        ...
        return self._coefficients.gamma_s
    ...
    def gamma_s_report(self) -> FormulaResult:
        ...
        return FormulaResult(
            ...
            formula_values=f"γs = {c.gamma_s:.2f}",
            result=c.gamma_s,
            unit="-",
    ...
    def fyd(self) -> float:
        ...
        return self._fyk / self._coefficients.gamma_s
    ...
    def fyd_report(self) -> FormulaResult:
        ...
        return FormulaResult(
            name="fyd",
            formula="fyd = fyk / γs",
            formula_values=f"fyd = {self._fyk:.2f} / {c.gamma_s:.2f} = {self.fyd:.2f}",
            result=self.fyd,
            unit="MPa",
    ...
    def ftd(self) -> float:
        ...
        return self.ftk / self._coefficients.gamma_s
    ...
    def ftd_report(self) -> FormulaResult:
        ...
        return FormulaResult(
            name="ftd",
            formula="ftd = ftk / γs",
            formula_values=f"ftd = {self.ftk:.2f} / {c.gamma_s:.2f} = {self.ftd:.2f}",
            result=self.ftd,
            unit="MPa",
            ref=f"EC2 — §3.2.7(2) — AN {c.country}",
        )
